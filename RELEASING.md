...
Workflow `Publish Maven package` проверит историю с помощью Gitleaks, выполнит
принадлежность release-коммита ветке `main`, тесты, SpotBugs и Trivy, а затем
запустит `mvn deploy`. Публикация содержит
основной JAR, исходники и JavaDoc. Убедитесь, что workflow завершился успешно и
версия появилась в разделе `Packages`. Только после этого обновляйте версию в
потребителях.
...
ветке `main` правило, запрещающее force push и требующее pull request с одним
одобрением. В `Require status checks to pass` добавьте проверки:
...
отдельное правило, запрещающее их изменение и удаление. Если GitHub предлагает
защиту deployment environment, создайте окружение `github-packages`, потребуйте
ручное подтверждение и укажите его в job `publish` через
...
commit SHA и tag SHA. GitHub запрещает самостоятельное удаление публичной версии
с более чем 5000 загрузок.

Для REST API нужен classic PAT: `read:packages` и `delete:packages` для
удаления, `read:packages` и `write:packages` для восстановления. Удалённую
версию можно восстановить в течение 30 дней, если её пространство имён ещё
свободно. См. [официальную документацию GitHub](https://docs.github.com/en/packages/learn-github-packages/deleting-and-restoring-a-package).
